...
规范化只忽略 SQL 关键字大小写、注释及引号外空白，不改变引号内字面量。只有完整匹配由同一份 v1
DDL 生成的期望指纹后，才能收紧文件权限、启用 WAL/secure-delete 或执行 DDL。SQLite 文件由引擎
独占，不允许混放采用方对象或业务数据；空 SQLite 文件没有已有业务对象，可作为显式初始化目标。
该顺序防止路径误配、约束被移除或同名伪装数据库先被引擎修改、随后才失败。
